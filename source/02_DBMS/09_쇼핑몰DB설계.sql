...
SELECT * FROM ORDERS;
-- 2-2. ORDER_DETAIL(주문상세) 테이블(장바구니에 넣은 물건을 한 번에 주문할 경우)
-- 주문상세 테이블에 INSERT할 때 필요한 서브쿼리 연습
SELECT ORDER_DETAIL_SEQ.NEXTVAL odNO, TO_CHAR(SYSDATE, 'RRMMDD') || TRIM(TO_CHAR(ORDERS_SEQ.CURRVAL, '000')) ONO, PCODE, QTY, COST
    FROM CART WHERE mID = 'abc';    -- 주문상세 테이블에 INSERT 할 때 필요한 서브쿼리
DROP SEQUENCE ORDER_DETAIL_SEQ;
CREATE SEQUENCE ORDER_DETAIL_SEQ MAXVALUE 999999999 NOCYCLE NOCACHE;
INSERT INTO ORDER_DETAIL (odNO, oNO, pCODE, QTY, COST)
    SELECT ORDER_DETAIL_SEQ.NEXTVAL odNO, TO_CHAR(SYSDATE, 'RRMMDD') || TRIM(TO_CHAR(ORDERS_SEQ.CURRVAL, '000')) oNO, pCODE, QTY, COST
    FROM CART WHERE MID = 'abc';
SELECT * FROM ORDER_DETAIL; -- ORDERS 테이블과 ORDER)DEATIL테이블 완성

-- 3. 재고수량 수정(A1상품 3개, B1상품 1개 구매하여 재고 수정)

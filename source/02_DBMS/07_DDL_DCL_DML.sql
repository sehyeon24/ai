-- [VII] DDL, DCL, DML
-- SQL = DDL(테이블 생성, 테이블 삭제, 테이블 구조 변경, 테이블 모든 데이터 제거, 테이블명 변경) + : ROLLBACK(삭제) 불가능
--       DML(SELECTM INSERT, UPDATE, DELETE) + : ROLLBACK 가능
--       DCL(사용자 계정 생성, 사용자에게 권한 부여, 권한 박탈, 사용자 계정 삭제, 트랜잭션 명령어) : ROLLBACK(삭제) 불가능
-- ★★★ DDL ★★★ --
...
CREATE TABLE EMP03  -- EMP의 특정 필드만
    AS
    SELECT EMPNO, ENAME, DEPTNO FROM EMP;
SELECT * FROM EMP03;

CREATE TABLE EMP04  -- EMP의 특정 행만(서브쿼리의 WHERE 절로 제한)
    AS
...
-- (1) 필드 추가(ADD)
SELECT * FROM EMP03;    -- EMPNO(숫4), ENAME(문10), DEPTNO(숫2)
DESC EMP03;             -- 필드 타입 확인
ALTER TABLE EMP03 ADD (JOB VARCHAR2(20), SAL NUMBER(7, 2)); -- 2개 이상 추가시 () 안에
SELECT * FROM EMP03;    -- 추가된 필드의 데이터는 자동으로 NULL
ALTER TABLE EMP03 ADD COMM NUMBER(7, 2);
-- (2) 필드 수정(MODIFY)
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(4);     -- 숫자데이터가 들어있으면 숫자로만 변경 가능(문자로 변경 불가능)
ALTER TABLE EMP03 MODIFY EMPNO NUMBER(5);       -- 숫자에서 자릿수 늘리기 가능
ALTER TABLE EMP03 MODIFY EMPNO NUMBER(4);       -- 숫자는 줄이기 불가능
ALTER TABLE EMP03 MODIFY ENAME VARCHAR2(100);
ALTER TABLE EMP03 MODIFY ENAME VARCHAR2(10);    -- 문자 데이터 필드는 늘이거나 줄이기 가능
...
ALTER TABLE EMP03 MODIFY (SAL VARCHAR2(10), COMM NUMBER(3));    -- NULL필드는 마음대로 수정 가능 // 2개 이상 추가시 () 안에
-- (3) 필드 삭제(DROP COLUMN)
ALTER TABLE EMP03 DROP COLUMN JOB;  -- JOB : NULL이었던 필드
SELECT * FROM EMP03;
...
SELECT * FROM EMP03;
DROP TABLE DEPT01;  
-- EMP01테이블에서 DEPT테이블을 참조할 경우 EMP01을 삭제한 후 DEPT01테이블 삭제 가능
DROP TABLE EMP01;
DROP TABLE DEPT01;

-- 4. 테이블 모든 데이터 제거(TRUNCATE TABLE 테이블명)
SELECT * FROM EMP02;
TRUNCATE TABLE EMP02;   -- ROLLBACK 불가능
SELECT * FROM EMP02;
...
-- 6. 데이터 딕셔너리(DB자원을 효율적으로 관리하기 위한 시스템 테이블 : 일반사용자 접근불가)
--    VS. 데이터 딕셔너리뷰(접근가능한 읽기전용 가상의 테이블)
-- 데이터 딕셔너리 뷰의 종류
    -- (1) USER_XXX : 현 계정이 소유하고 있는 객체(XXX : 테이블, 제약조건, 뷰, 인덱스)
...
-- ★★★ DML ★★★ --    
-- 1. INSERT INTO 테이블명 VALUES (값1, 값2, ..._); 모든 필드 데이터 입력
    -- INSERT INTO 테이블명 (필드명1, 필드명2, ,,,) VALUES (값1, 값2, ...); 언급된 필드 외는 NULL로 입력됨
    SELECT * FROM DEPT01;   -- 테이블
    INSERT INTO DEPT01 VALUES (50, 'ACCOUNTING', 'NEW YORK');
    INSERT INTO DEPT01 VALUES (60, 'SALES', NULL);  -- 명시적 NULL 데이터 추가
    INSERT INTO DEPT01 (DEPTNO, DNAME, LOC) VALUES (70, 'RESEARCH', '신림');  -- 필드명 순서 관계 없음
    INSERT INTO DEPT01 (LOC, DNAME, DEPTNO) VALUES ('신길', 'IT', 80);
    INSERT INTO DEPT01 (DEPTNO, DNAME) VALUES (90, 'OPERATION');    -- 묵시적으로 NULL 추가
    SELECT * FROM DEPT01;
    -- 서브쿼리를 이용한 INSERT
    -- INSERT INTO 주문테이블 SELECT * FROM 장바구니 WHERE MEMBERID = '내 아이디';
    INSERT INTO DEPT01 SELECT * FROM DEPT WHERE DEPTNO < 40;
    SELECT * FROM DEPT01;
        -- EX. BOOK(ID는 11, 책이름은 스포츠의 의학, 출판사는 한솔출판, 출판일 오늘, 가격은 90,000)(SYSDATE)
        SELECT * FROM BOOK; 
        INSERT INTO BOOK VALUES (11, '스포츠의 의학', '한솔출판', SYSDATE, TO_NUMBER('90,000', '99,999'));
        -- EX. BOOK(ID는 12, 책이름은 스포츠의 과학, 출판사는 NULL, 출판일 오늘, 가격은 50,000)(SYSDATE)    
        INSERT INTO BOOK (BOOKID, BOOKNAME, RDATE, PRICE) VALUES (12, '스포츠의 과학', SYSDATE, 50000);
        SELECT * FROM BOOK;
    -- 트랜젝션 명령어 : DML명령어들을 DB에 적용(COMMIT) + DML명령어들 취소(ROLLBACK)
    COMMIT;
        INSERT INTO BOOK (BOOKID, BOOKNAME, RDATE, PRICE) VALUES (13, '스포츠의 과학', SYSDATE, 50000);
...
-- 2. UPDATE 테이블 SET 필드명1 = 값1 [, 필드명2 = 값2, ... 필드명N = 값N] [WHERE 조건];
DROP TABLE EMP01;
CREATE TABLE EMP01 AS SELECT EMPNO, ENAME, HIREDATE, SAL, DEPTNO FROM EMP;
    -- EX. 부서번호를 30으로 수정하시오.
    UPDATE EMP01 SET DEPTNO = 30;   -- WHERE조건 절이 없으므로 모든 부서번호가 변경됨
    SELECT * FROM EMP01;
    ...
    SELECT * FROM EMP01;
    -- EX. DALLAS에 근무하는 직원의 급여를 1000$ 인상시키시오.(서브쿼리 이용)
    UPDATE EMP01 SET SAL = SAL+1000 
        WHERE DEPTNO = (SELECT DEPTNO FROM DEPT WHERE LOC = 'DALLAS');
    SELECT * FROM EMP01;
    -- EX. EMP에서 SCOTT의 부서번호는 30, SAL과 COMM은 500$씩 인상, JOB은 'MANAGER', 상사를 'KING'으로 수정하시오.
    COMMIT;
    UPDATE EMP SET DEPTNO = 30, 
                     SAL = SAL +500, 
                     COMM = NVL(COMM, 0) + 500,
                     JOB = 'MANAGER',
                     MGR = (SELECT EMPNO FROM EMP WHERE ENAME = 'KING') -- 서브쿼리 _____________________________________________
            WHERE ENAME = 'SCOTT';
    SELECT * FROM EMP WHERE ENAME = 'SCOTT';
    ROLLBACK;
    -- DEPT01에서 60번 부서의 지역명을 20번 부서 지역명으로 수정하시오.
    UPDATE DEPT01 SET LOC = (SELECT LOC FROM DEPT01 WHERE DEPTNO = 20)
        WHERE DEPTNO = 60;
    SELECT * FROM DEPT01;
    -- EMP01에서 모든 사원의 급여와 입사일을 'KING'의 급여와 입사일로 수정
    UPDATE EMP01 SET SAL = (SELECT SAL FROM EMP01 WHERE EANME = 'KING'),
...
SELECT * FROM EMP01;
DELETE FROM EMP01;  -- DML명령어는 취소 가능(DDL은 취소 불가능)
SELECT * FROM EMP01;    
ROLLBACK;
SELECT * FROM EMP01;
SELECT * FROM DEPT01;
...
-- DELETE시 다른 테이블에서 참조하는 데이터는 삭제 불가능
DROP TABLE EMP01 CASCADE CONSTRAINTS;   -- 참조 테이블이 있어도 강제로 테이블 삭제
...
DELETE FROM DEPT01;     -- 참조 데이터가 있을 경우 에러
    ...
    -- SAM01 : JOB이 정해지지 않는 사원을 삭제하시오.(NULL값인 것)
    DELETE FROM SAM01 WHERE JOB IS NULL;
    -- SAM01 : ENAME이 ORANGE인 사원을 삭제하시오.
...
UPDATE EMP01 SET SAL = SAL*1.1 WHERE SAL >= 3000;
-- 4.
UPDATE EMP01 SET SAL = (SAL*12)+1000 WHERE DEPTNO = (SELECT DEPTNO FROM DEPT WHERE LOC = 'DALLAS');
-- 5.
UPDATE EMP01 SET (DEPTNO = 20, JOB = 'MANAGER') = (SELECT DEPTNO, JOB FROM EMP01 WHERE ENAME = 'SCOTT');
-- 6. 
DELETE FROM EMP01 WHERE DNAME = 'SALES';
-- 7.
...
-- (4) UNIQUE : 모든 행의 값이 유일. NULL값은 허용(NULL은 여러 개 입력 가능)
...
INSERT INTO EMP1 (EMPNO, ENAME, JOB, DEPTNO) 
    VALUES (1001, 'HONG', 'MANAGER', 10);       -- 에러(DEPT1테이블에 10번 부서가 없으므로 참조 오류)
INSERT INTO DEPT1 SELECT * FROM DEPT;
SELECT * FROM DEPT;
INSERT INTO DEPT1 (DEPTNO, LOC, DNAME) VALUES (40, '신림', 'IT'); -- PRIMARY KEY 제약조건(CONSTRAINT) 오류
INSERT INTO DEPT1 VALUES (50, 'SALES', '신림');   -- UNIQUE 제약조건 오류
INSERT INTO DEPT1 VALUES (50, 'IT', NULL);      -- NOT NULL 제약조건 오류
INSERT INTO DEPT1 (DEPTNO, DNAME) VALUES (50, 'IT');    -- NOT NULL 오류
SELECT * FROM EMP1;
INSERT INTO EMP1 (EMPNO, ENAME, JOB, DEPTNO)
    VALUES (1001, 'HOGN', 'MANAGER', 10);   -- HIREDATE에는 DEFAULT값 입력됨
INSERT INTO EMP1 (EMPNO, JOB, DEPTNO) VALUES (1002, 'MANAGER', 10); -- NOT NULL 오류
INSERT INTO EMP1 (EMPNO, ENAME, JOB, SAL, COMM, DEPTNO)
    VALUES (1002, 'LEE', 'MANAGER', 10, NULL, 20);   -- CHECK(SAL>0) 오류
INSERT INTO EMP1 (EMPNO, ENAME, JOB, SAL, COMM, DEPTNO)
    VALUES (1002, 'LEE', 'MANAGER', 10, -1, 20);   -- CHECK(SAL>0) 오류
...
DROP USER scott2 CASCADE;   -- 사용자 계정 삭제(접속 불가)

-- == ※ DML 연습문제 (pdf 4~5페이지) ※ ==
DROP TABLE BOOK;
DROP TABLE BOOKCATEGORY;
SELECT * FROM TAB;          -- 확인(딕셔너리뷰)
SELECT * FROM USER_TABLES;  -- 확인(딕셔너리뷰)
...
CREATE TABLE BOOK (
    ...
    PUBYEAR     NUMBER(4) DEFAULT TO_CHAR(SYSDATE, 'YYYY'),   -- =DEFAULT EXTRACT(YEAR FROM SYSDATE)
...
INSERT INTO BOOK (cCODE, bNO, bNAME, PUBLISHER)     -- PUBYEAR DEFAULT 값 확인
...
CREATE TABLE MAJOR (
    ...
    mNAME   VARCHAR2(100) UNIQUE,
    mOFFICE VARCHAR2(100)
);
CREATE TABLE STUDENT (
    sNO     NUMBER(3) PRIMARY KEY,
    sNAME   VARCHAR2(100),
    sSCORE  NUMBER(3) CHECK(0 < sSCORE AND sSCORE <= 100),
    mCODE   NUMBER(1),
...
SELECT sNO, sNAME, sSCORE, M.mCODE, mNAME, mOFFICE      -- =S.*
    FROM MAJOR M, STUDENT S
    WHERE M.mCODE = S.mCODE;
UPDATE STUDENT SET sNO = 100, mCODE = 2
    WHERE sNO = '101';                                  -- UPDATE 문법
SELECT * FROM STUDENT;

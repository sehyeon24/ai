...
-- 서브쿼리 종류(1) 단일행 서브쿼리 : 서브쿼리 결과가 단일행 : =, >, >=, <, <=, !=
    -- EX. SCOTT과 동일한 부서번호에서 근무하는 사원의 이름과 급여를 출력하시오.
...
-- 서브쿼리 종류(2) 다중행 서브쿼리 : 서브쿼리 결과가 2행이상 : IN, >ALL, ANY(SOME), EXISTS
    -- EX. SCOTT이나 KING과 동일한 부서번호에서 근무하는 사원의 정보를 출력하시오.
...
-- ★ 2. 단일행 서브쿼리
    ...
    SELECT LOC FROM EMP E, DEPT D
        WHERE E.DEPTNO = D.DEPTNO AND ENAME = 'SCOTT';  -- 단일행 서브쿼리
    SELECT * FROM EMP E, DEPT D
        WHERE E.DEPTNO = D.DEPTNO
            AND LOC = (SELECT LOC FROM EMP E, DEPT D
                WHERE E.DEPTNO = D.DEPTNO AND ENAME = 'SCOTT')
            AND ENAME <> 'SCOTT';   -- SCOTT제외
    ROLLBACK;
    ...
    SELECT SUM(SAL) FROM EMP
        WHERE DEPTNO = (SELECT DEPTNO FROM EMP WHERE ENAME = 'SCOTT');
    -- EX6. DALLAS에 근무하는 사원의 이름과 부서번호를 출력하시오. (서브쿼리, JOIN)
    ...
    SELECT ENAME, SAL, ROUND((SELECT AVG(SAL) FROM EMP))-SAL "DIFF"
        FROM EMP;
    -- 단일행 다중열 서브쿼리
    -- EX. SCOTT과 JOB도 같고 부서번호도 같은 직원의 모든 필드를 출력하시오.
...
-- (1) ALL : 서브쿼리 결과가 모두 만족하면 참
    ...
    SELECT SAL FROM EMP WHERE DEPTNO = 30;  -- 다중행 서브쿼리 950, 1250, 1500, 1600, 2850
    SELECT * FROM EMP WHERE SAL > ALL(SELECT SAL FROM EMP WHERE DEPTNO = 30);  -- 메인쿼리
                                -- > 연산자는 단일행 함수에서만 적용가능한 것으로 서브쿼리 앞에 ALL 사용
    -- (== 30번 부서 최대 급여보다 큰 직원의 모든 필드를 출력하시오.) : 단일행 서브쿼리
    SELECT * FROM EMP WHERE SAL > (SELECT MAX(SAL) FROM EMP WHERE DEPTNO = 30);
-- (2) ANY(=SOME): 서브쿼리 결과가 하나라도 만족하면 참
    -- EX. 30번 부서 직원 한 명의 급여보다 큰 직원의 모든 필드를 출력하시오. (최소값보다 크면 됨)
    SELECT SAL FROM EMP WHERE DEPTNO = 30;  -- 다중행 서브쿼리 950, 1250, 1500, 1600, 2850
    SELECT * FROM EMP WHERE SAL > ANY(SELECT SAL FROM EMP WHERE DEPTNO = 30);
    -- (== 30번 부서 최소 급여보다 큰 직원의 모든 필드를 출력하시오.) : 단일행 서브쿼리   
    SELECT * FROM EMP WHERE SAL > (SELECT MIN(SAL) FROM EMP WHERE DEPTNO = 30);
-- (3) IN
    -- EX. 부서별로 입사일이 가장 늦은 사람의 이름, 입사일, 부서번호를 출력하시오.
    ...
    -- 급여를 3000이상 받는 사원이 소속된 부서 직원의 모든 필드를 출력하시오.
...
-- (4) EXISTS : 서브쿼리 결과가 존재하면 참
    -- EX. 직속부하가 있는 직원들의 사번, 이름, 급여를 출력하시오.
    SELECT DISTINCT M.EMPNO, M.ENAME, M.SAL     --  DISTINCT : 중복 제거
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO;      -- SELF JOIN 이용
    SELECT EMPNO, ENAME, SAL FROM EMP MANAEGER
                -- 읽어들인 EMP데이터의 EMPNO가 MGR에 존재하는지
        WHERE EXISTS (SELECT * FROM EMP WHERE MANAGER.EMPNO = MGR);     -- 서브쿼리 이용
    -- 직속부하가 없는 직원들의 사번, 이름, 급여를 출력하시오.
    ...
    -- 분기 : 월 별로 /3한 값 반올림
    SELECT HIREDATE, CEIL(EXTRACT(MONTH FROM HIREDATE)/3) "QUARTER" FROM EMP;   -- 분기 // CEIL 올림
    ...
    SELECT TO_CHAR(HIREDATE, 'Q') "QUARTER", MAX(SAL) 
        FROM EMP GROUP BY TO_CHAR(HIREDATE, 'Q');   -- 다중열 다중행 서브쿼리
    SELECT  TO_CHAR(HIREDATE, 'Q') "QUARTER", EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM
        ...
        ORDER BY QUARTER;                           -- 메인쿼리 // SELECT절의 alias 사용
    ...
    SELECT ENAME, SAL, JOB FROM EMP 
        WHERE SAL > ALL(SELECT SAL FROM EMP WHERE JOB = 'SALESMAN');    -- 메인쿼리
        -- = JOB이 SALESMAN인 사원들 최대 급여보다 많이 받는 사원 정보를 출력하시오.
    SELECT ENAME, SAL, JOB FROM EMP 
...
-- 23. 부서 평균 월급보다 월급이 높은 사원을 사번, 이름, 급여, 부서번호
    SELECT EMPNO, ENAME, SAL, E.DEPTNO
        ...
        WHERE E.DEPTNO = G.DEPTNO
            AND SAL > AVGSAL;   -- EQUI JOIN과 FROM절의 서브쿼리를 이용
...
SELECT DEPTNO, ENAME, SAL
    FROM EMP E
    WHERE SAL>(SELECT AVG(SAL) FROM EMP WHERE DEPTNO=E.DEPTNO);
-- 24-1. 업무별로 평균 월급보다 적은 월급을 받는 사원을 부서번호, 이름, 급여, JOB, JOB평균
...
SELECT ENAME, JOB, SAL
    ...
    WHERE DEPTNO = (SELECT DEPTNO FROM DEPT 
                        WHERE INITCAP(LOC) = 'Dallas');     -- SUBQUERY 이용
...
SELECT * FROM EMP
    WHERE SAL > ALL(SELECT MIN(SAL) FROM EMP WHERE DEPTNO = 30);    -- 단일행 서브쿼리
SELECT * FROM EMP 
...
SELECT ENAME, JOB, SAL
    FROM EMP
    WHERE SAL IN (SELECT SAL FROM EMP WHERE UPPER(ENAME) IN('SCOTT', 'WARD'))
...
SELECT JOB, ENAME, EMPNO, DEPTNO
    ...
    -- (MGR출력)서브쿼리 결과 : 7839, NULL, 7782, 7698, 7902, 7566, 7788

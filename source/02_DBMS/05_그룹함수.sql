...
-- ★ 1. 그룹함수들 실습
SELECT ROUND(AVG(SAL), 2) FROM EMP;    -- 싱글라인(소숫점 두 자리에서 반올림)
SELECT AVG(SAL), SUM(SAL)/COUNT(SAL) FROM EMP;  --COUNT(행(데이터)의 갯수)
SELECT * FROM EMP;
...
SELECT ROUND(AVG(SAL), 1), SUM(SAL), MIN(SAL), MAX(SAL), 
    ...
    SELECT 
        ...
        MIN(ENAME), COUNT(JOB)  -- 아스키코드 가장 작은 것 // NULL제외
        FROM EMP;
    -- 탄탄EX1. 입사한지 가장 오래된 사원의 입사일과 가장 최근 입사일을 출력하시오.
    ...
    SELECT
        ...
        FROM EMP;
    -- 탄탄EX3. 결과 80년 12월 17일 최초입사 : 18,123일째    83년 01월 12일 최근입사 : 16,123일째
    SELECT
    ...
    SELECT * FROM EMP WHERE DEPTNO = 10;    -- 0 확인
    SELECT COUNT(COMM) FROM EMP WHERE DEPTNO = 10;  -- COMM이 0인 행을 포함
    
-- ★ 2. GROUP BY 절 : 그룹함수를 쓰되, 어떤 컬럼 기준으로 그룹함수를 적용할 것인가
    -- EX1. 부서번호별 최대급여, 급여평균, 급여표준편차를 출력하시오. (부서번호별로 정렬)
    SELECT DEPTNO NO, MAX(SAL), AVG(SAL), STDDEV(SAL) 
        FROM EMP 
        GROUP BY DEPTNO     -- GROUP BY 절은 필드의 별칭 사용 불가 (엘리엇 X)
        ORDER BY NO;        -- ORDER BY 절은 필드의 별칭 사용 가능
    -- 부서명별 최대급여, 평균급여 (평균급여순 정렬:오름차순)
    SELECT DNAME, MAS(SALL), AVG(SAL)
        FROM EMP E, EPT D
...
-- ★ 3. HAVING 절 : 그룹함수의 결과를 제한할 떄
    ...
    SELECT DEPTNO, COUNT(*), MAX(SAL), MIN(SAL), AVG(SAL)
        ...
        ORDER BY AVG(SAL);          -- ORDER BY 절에 필드 별칭 사용 가능
    -- EX. SAL이 5000미만인 사원에 대해 부서이름별 급여평균과 표준편차를 소수점 2자리까지 출력하시오.
...
SELECT MAX(SAL)-MIN(SAL) FROM EMP GROUP BY SAL;
-- 4. 각 부서별로 인원수, 급여 평균, 최저 급여, 최고 급여, 급여의 합을 출력(급여의 합이 많은 순으로)
SELECT E.DEPTNO, COUNT(*), AVG(SAL), MIN(SAL), MAX(SAL), SUM(SAL)
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
    GROUP BY E.DEPTNO
    ORDER BY SUM(SAL);
...
SELECT JOB, E.DEPTNO, COUNT(*), AVG(SAL), SUM(SAL)
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
...
SELECT E.DEPTNO, AVG(SAL), SUM(SAL), MIN(SAL)
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
    GROUP BY E.DEPTNO;

...
show databases;
use dev_user;				-- dev_user 데이터베이스 영역으로 들어감
show tables;
...
select * from emp;
drop table if exists emp;	-- 해당 테이블이 있으면 지우는 명령어

-- major : mCode(학과번호, 인위적인 pk), mName(학과이름), mOffice(학과사무실)
-- student : sNo(학번, pk), sName(이름), mCode(학과번호, fk)
-- create sequence major_seq maxvalue 9999;		// oracle
create table major(
	mCode	int primary key auto_increment,	-- 시퀀스 없이 자동 증가되는 pk(int형)
    mName	varchar(10),
    mOffice	varchar(30)    
);
...
create table major(							-- pk 제약조건은 옆에 써도 무방
	mCode	int primary key auto_increment,	-- 시퀀스 없이 자동 증가되는 pk(int형)
    mName	varchar(10),
    mOffice	varchar(30)    
);
create table student(
    ...
    sNAme	varchar(10),
    mCode	int,
    foreign key(mCode) references major(mCode)	-- 외래키 제약조건은 반드시 아래에 작성
);
insert into major (mName, mOffice) values ('컴공', '201호');		-- ctrl+d복사
insert into major (mName, mOffice) values ('빅데이터', '202호');	-- ctrl+d복사
insert into major (mName, mOffice) values ('웹디', '203호');		-- ctrl+d복사
...
insert into student values (1001, '홍길동', 1);
insert into student values (1002, '신길동', 2);
insert into student values (1011, '박길동', 4);	-- 외래키 제약조건 에러
select * from student;
...
select sno, sname, s.mcode, mname, moffice
    ...
    on s.mcode = m.mcode;	-- from과 테이블 순서 일치해야 함
...
insert into division values (20, 'research', '02-777-88808', '용산');
...
-- ifnull(널일 수도 있는 필드, 널 대체값)	-- 매개변수 2개의 타입은 일치하지 않아도 됨	// NVL
select pno "no", pname, pay, bonus, pay+ifnull(bonus, 0) sum from personal;
-- if(조건, 조건이 참일 때 반환값, 조건이 참이 아닐 때 반환값)
select pname, pay+if(bonus is null, 0, bonus) from personal;
...
select w.pno, w.pname, m.pname "manager"			-- 7번 활용
...
select w.pno, w.pname, m.pname, dname
	from division d, personal w right outer join personal m
    on w.manager = m.pno
    where w.dno = d.dno;
...
select avg(pay) from personal;	-- 회사 평균 급여
...
select dno, avg(pay) 
	from personal 
    group by dno;				-- 부서별 평균급여
  -- 서브쿼리 이용
select pname, pay, dno, 
		round((select avg(pay) from personal where dno = p.dno)) avg	-- where절에 alias주고 생략 가능
    from personal p
...
select pname, dname, pay, (select avg(pay) from personal where dno=p.dno)
	from personal p, division d
    where p.dno = d.dno;
	-- 서브 쿼리 이용
select pname, pay, dname, 
...
-- Oracle에서의 단일행 함수와 다른 부분
	-- ex. pname는 job이다
select concat(pname || '는', job, '다') "message" from personal;	-- ||가 연결연산자가 아니므로 concat 사용
select round(35.78, 1);		-- from절 없이도 실행 가능(dual테이블 지원불가)

-- 시스템으로부터 현재시점, 현재날짜, 현재시간
select sysdate(), now();	-- 현재 날짜와 시간
select curdate(), curtime();	-- 현재 날짜와 현재 시간

-- date_format(날짜형, 포맷) => 날짜/시간형을 문자형으로 변환
-- date_format(문자, 포맷) 가능 => 문자를 날짜/시간형으로 변환
...
select year(now()), weekday(now()) "요일";	-- 0: 월요일, 1: 화요일, ...
...
-- pay순으로 1page (1~3등까지)
select pname, pay from personal order by pay desc limit 0, 3;	-- 0생략 가능(1등부터이기 때문에)
-- 2page (4~6등까지)

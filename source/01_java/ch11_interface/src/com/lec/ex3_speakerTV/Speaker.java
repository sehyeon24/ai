...
public class Speaker implements IVolume {
	...
	public void volumeUp(int i) {	// 매개변수 있는 : 볼륨을 i씩 up
		if(volumeLevel == MAX_VOLUME) {
			System.out.println("스피커 볼륨이 최대치(" + MAX_VOLUME + ")이어서 못 올려요");			// 경우의 수 1 : 증가 못하는 경우
		}else if( (volumeLevel+i) <= MAX_VOLUME) {
			volumeLevel += i;
			System.out.println("스피커 볼륨을 " + i + "만큼 올려 현재 볼륨은 " + volumeLevel);		// 경우의 수 2 : i만큼 증가 가능한 경우
		}else {
			int temp = MAX_VOLUME - volumeLevel;	// temp : 볼륨을 내가 얼만큼 올렸는지(현재 증가 가능한 정도)
			volumeLevel = MAX_VOLUME;				//	한 번에 씩 못 올리면 다 MAX_VOLUME
			System.out.println("스피커 볼륨이 " + temp + "만큼 올려 최대치입니다");					// 경우의 수 3 : i만큼 증가 못하는 경우 
		}	// if
	}	// volumeUp
	...
	public void volumeDown(int i) {	// 볼륨을 i씩 down
		if(volumeLevel == MIN_VOLUME) {
			System.out.println("스피커 볼륨이 최소치(" + MIN_VOLUME + ")이어서 못 내려요");
			...
			System.out.println("스피커 볼륨이 " + temp + "만큼 내려 최소치입니다");
		}	// if
	}	// volumeDown
}	// class

...
public class StudentTestMain {
	private static void line() {	// overloading 오버로딩
		for(int i=0 ; i<65 ; i++) {
			System.out.print('■');
		}
		System.out.println(); // 개행
	}
	private static void line(char ch) {	// 매개변수 있는 line(ch);
		System.out.print("\t");
		for(int i=0 ; i<50 ; i++) {
			System.out.print(ch);
		}
	...
	public static void main(String[] args) {
		...
		line('-');
		for(String title : titles) {	//확장for문
		...
		for(Student student : students) {
			...
			//tot[4] = (int)(tot[4] + student.getAvg()); : 위와 같음
		}
		line('~');
		System.out.print("\t총점");
		for(int t : tot) {
